(() => {
  ...
  var getMatchingDayPair = (day, daysConf) => {
    const dayGroup = [];
  ...
  var shouldShowDayAsOpen = (day, daysConf) => {
  ...
  var shouldShowDayAsClosed = (day, daysConf) => {
    const openTime = getMatchingDayPair(day, daysConf)[0]?.time;
    const closeTime = getMatchingDayPair(day, daysConf)[1]?.time;
    return !!(openTime === "closed" && closeTime === "closed");
  };
  var shouldRenderInput = (day, daysConf) => {
    return shouldShowDayAsOpen(day, daysConf);
  };
  var shouldDisableTimeOption = (timeOption, day, daysConf) => {
    const openTime = getMatchingDayPair(day, daysConf)[0]?.time;
    const closeTime = getMatchingDayPair(day, daysConf)[1]?.time;
    const baseDate = /* @__PURE__ */ new Date("1970-01-01");
    const dateTimeOpen = /* @__PURE__ */ new Date(`${baseDate.toDateString()} ${openTime}`);
    const dateTimeClose = /* @__PURE__ */ new Date(`${baseDate.toDateString()} ${closeTime}`);
    const dateTimeWithTimeOption = /* @__PURE__ */ new Date(`${baseDate.toDateString()} ${timeOption.value}`);
    if (day.id.split("_")[1] === "open") {
      return dateTimeWithTimeOption >= dateTimeClose;
    }
    return dateTimeWithTimeOption <= dateTimeOpen;
  };
  var OpeningHoursUnstyled = (props) => {
    const {
      getValues,
      values,
      ampm,
    ...
    const [formValues, setFormValues] = (0, import_react.useState)(values);
    const [daysConfig, setDaysConfig] = (0, import_react.useState)([
    ...
    const copyToAll = (dayToCopy) => {
      ...
      const newDaysConfig = daysConfig.map((day, i, arr) => {
        if (day.id.split("_")[1] === "open" && shouldShowDayAsOpen(day, arr) && day.id !== openDay?.id) {
          setFormValues((prev) => {
            return { ...prev, [day.id]: openDay?.time };
          });
          return { ...day, time: openDay?.time };
        } else if (day.id.split("_")[1] === "close" && shouldShowDayAsOpen(day, arr) && day.id !== closeDay?.id) {
          setFormValues((prev) => {
    ...
    return /* @__PURE__ */ React.createElement(
      "div",
      {
        ...
        style: rootContainerStyles ? rootContainerStyles : !rootContainerClassName ? {
          display: "flex",
      ...
      /* @__PURE__ */ React.createElement("div", { className: dayButtonContainerClassName, style: dayButtonContainerStyles }, daysConfig && daysConfig.map((day, i, arr) => {
        if (shouldShowDayAsOpen(day, arr) && day.id.split("_")[1] !== "close") {
          return /* @__PURE__ */ React.createElement(
            ...
            /* @__PURE__ */ React.createElement("span", null, day.label[0])
          );
          ...
          return /* @__PURE__ */ React.createElement(
            "button",
            {
              type: "button",
              key: `${day.id}${i}-btn`,
              style: dayButtonInactiveElementStyles ? dayButtonInactiveElementStyles : dayButtonActiveElementClassName ? null : {
                opacity: 0.3
              },
      ...
      /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(
          ...
          daysConfig && daysConfig.map((day, i, arr) => {
            ...
            if (shouldRenderInput(day, arr) && i % 2 === 0) {
              return /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(
                  ...
                  selectComponent ? (0, import_react.cloneElement)(selectComponent, {
                    ...
                    timeOptions.map((t) => /* @__PURE__ */ React.createElement(
                      "option",
                      {
                        key: t.value,
                        value: t.value,
                ...
                showCopyToAll && arr.filter((x, i2, r) => shouldRenderInput(x, r)).sort((a, b) => a.seq - b.seq)[1]?.id === close.id ? /* @__PURE__ */ React.createElement(
                  "div",
                  {
                    key: `${close.id}-${i}-div-copy`,
                    style: copyButtonContainerStyles ?? {
                      position: "absolute",
                      right: -60,
                      top: 0
                    },
                  ...
                  /* @__PURE__ */ React.createElement("div", null, copyToAllButtonComponent ? (0, import_react.cloneElement)(copyToAllButtonComponent, {
  ...
  var OpeningHoursUnstyled_default = OpeningHoursUnstyled;
})();
